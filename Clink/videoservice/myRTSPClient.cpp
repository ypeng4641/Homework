...
extern RtspThreadContext g_context[];
...
void continueAfterGETPARAMETER(RTSPClient* rtspClient, int resultCode, char* resultString) {
	printf("=========== GET_PARAMETER RESPONSE CODE : %d ==============\n", resultCode);	
}
...
void sendGetParameterCommand(RTSPClient* rtspClient)
{
	...
	while(pthread_mutex_lock(&g_context[channel].mutex) != 0 && errno == EINTR){};
  	...
  	if (scs.session != NULL) { 
   	   rtspClient->sendGetParameterCommand(*scs.session, continueAfterGETPARAMETER, NULL, NULL);
 	 }
